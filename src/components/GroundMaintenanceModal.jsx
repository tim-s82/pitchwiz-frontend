...
export default function GroundMaintenanceModal({ isOpen, onClose, venues, pitches, onMaintenanceCreated }) {
    ...
    const venuePitches = pitches.filter(p => p.venue === Number(selectedVenueId));
            ...
            <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                <div className="p-6 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center space-x-2.5">
                        ...
                        <h3 className="font-bold font-display text-slate-100">Schedule Ground Maintenance</h3>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition">
                        <X size={18} />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    {/* Venue Selection */}
                    <div>
                        <label className="block text-xs font-medium text-slate-400 mb-1.5">Venue <span className="text-red-400">*</span></label>
                        <select
                    ...
                    {selectedVenueId && (
                        <div>
                            <label className="block text-xs font-medium text-slate-400 mb-1.5">Affected Pitches <span className="text-red-400">*</span></label>
                            <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto p-2 rounded-xl bg-slate-950 border border-slate-800">
                                {venuePitches.length === 0 ? (
                                    <span className="text-xs text-slate-500 col-span-2 p-2">No pitches found for this venue.</span>
                                ) : (
                                    venuePitches.map(p => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`px-3 py-2 rounded-lg text-xs font-medium border text-left transition flex items-center justify-between ${isSelected ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' : 'bg-slate-900 text-slate-400 border-slate-800'
                                                    }`}
                                            >
                                                <span>{p.name}</span>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-medium text-slate-400 mb-1.5">Time Slot <span className="text-red-400">*</span></label>
                            <select
                                value={timeSlot}
                    ...
                    {/* Notes & Warning */}
                    <div>
                        <label className="block text-xs font-medium text-slate-400 mb-1.5">Maintenance Notes / Reason</label>
                        <textarea
                            value={notes}
                    ...
                    <div className="p-3 rounded-xl bg-emerald-950/30 border border-emerald-900/40 flex items-start gap-2.5 text-emerald-300 text-xs">
                        ...
                        <span><strong>Warning:</strong> Scheduling ground maintenance will automatically override and cancel any existing conflicting bookings on the selected pitches.</span>
                    </div>

                    <div className="flex justify-end gap-2 pt-2">
                        <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300 text-xs font-medium hover:bg-slate-700 transition">
                            Cancel
                        </button>
                        <button type="submit" disabled={saving || selectedPitches.length === 0} className="px-4 py-2 rounded-xl bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-500 transition disabled:opacity-50">
