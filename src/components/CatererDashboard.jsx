...
export default function CatererDashboard({
  ...
  const fetchCateringRequests = async () => {
    ...
    } catch (e) { console.warn('Could not fetch catering requests', e); }
  };
  ...
  const stats = useMemo(() => {
    ...
    return { teasCount, drinksCount, total: upcomingCatering.length, pendingCount };
  ...
  const submitCateringRejection = async () => {
    ...
    try {
      await fetch(`/api/catering-requests/${rejectModal.crId}/`, {
        ...
        body: JSON.stringify({ status: 'REJECTED', rejection_reason: rejectModal.reason })
      });
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-bold font-display text-slate-100">Caterer Dashboard</h2>
        ...
        <div className="flex gap-4">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl px-4 py-2 flex items-center space-x-2">
            ...
            <div>
              <span className="block text-xxs font-bold text-slate-400 uppercase tracking-wider font-display">Pending Approval</span>
              <span className="text-lg font-bold text-slate-100">{stats.pendingCount}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {pendingCateringRequests.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pendingCateringRequests.map(cr => {
                ...
                const pitchObj = booking ? pitches.find(p => p.id === booking.pitch) : null;
                ...
                const label = teamObj ? `${teamObj.name} vs ${fixtureObj.opponent}` : booking?.external_contact_name || 'External';

                return (
                  <div key={cr.id} className="glass-panel p-5 rounded-2xl border border-amber-900/40 bg-amber-950/5 space-y-4">
                    <div className="flex justify-between items-start">
                      <h4 className="font-bold text-slate-100 leading-snug font-display text-base">{label}</h4>
                      <div className="flex space-x-1.5 shrink-0">
                        {cr.requires_teas && (
                          <span className="bg-emerald-950/60 text-emerald-400 border border-emerald-900/50 text-[10px] font-extrabold font-display uppercase px-2 py-0.5 rounded">Teas</span>
                        )}
                        {cr.requires_drinks && (
                          <span className="bg-teal-950/60 text-teal-400 border border-teal-900/50 text-[10px] font-extrabold font-display uppercase px-2 py-0.5 rounded">Drinks</span>
                        )}
                      </div>
                    </div>
                    ...
                    {booking && (
                      <div className="space-y-1.5 text-xs text-slate-350">
                        <div className="flex items-center space-x-2">
                          ...
                          <span>{booking.start_date === booking.end_date ? booking.start_date : `${booking.start_date} to ${booking.end_date}`}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                    ...
                    <div className="flex gap-2 pt-1">
                      <button onClick={() => handleApproveCatering(cr.id)} className="flex-1 py-2 px-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold text-xs flex items-center justify-center gap-1 transition shadow-lg shadow-emerald-500/10">
                        <Check size={14} /> Approve
                      </button>
                      <button onClick={() => handleRejectCatering(cr.id)} className="flex-1 py-2 px-3.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-xs border border-slate-700 flex items-center justify-center gap-1 transition">
                        <X size={14} /> Reject
                      </button>
                    </div>
                  </div>
      ...
      {rejectModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 w-full max-w-md space-y-4">
            ...
            <p className="text-sm text-slate-400">A reason must be provided when rejecting a catering request.</p>
            <textarea
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setRejectModal({ open: false, crId: null, reason: '' })} className="px-4 py-2 rounded-lg bg-slate-800 text-slate-300 hover:bg-slate-700 text-sm font-semibold transition">Cancel</button>
              <button onClick={submitCateringRejection} className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white text-sm font-bold transition">Reject Request</button>
            </div>
          </div>
        </div>
      )}
